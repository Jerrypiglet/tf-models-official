...
def add_my_pose_loss_cars(FLAGS, prob_logits, labels, prob_logits_in_metric, labels_in_metric, masks_float, weights_normalized, balance_rot=1., balance_trans=1., upsample_logits=True, name=None, is_training_prefix='', loss_collection=None, if_depth=False, if_log_depth=True, logits_depth_N_log=None, labels_depth_N_log=None):
    ...
    trans_loss_error = tf.multiply(tf.abs(logits_depth_N_log - labels_depth_N_log), weights_normalized)
...
def logits_cls_to_logits_probReg(logits, bin_vals):
    bin_vals_expand = tf.tile(bin_vals, [tf.shape(logits)[0], 1])
