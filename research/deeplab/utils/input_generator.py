...
def _get_data(dataset, data_provider, dataset_split, codes_cons):
  ...
  if dataset_split != common.TEST_SET:
    image, vis, height, width, seg, shape_id_map_gt, pose_dict, shape_id_dict = data_provider.get([common.IMAGE, 'vis', common.HEIGHT, common.WIDTH, 'seg', 'shape_id_map', 'pose_dict', 'shape_id_dict'])
    vis = tf.reshape(vis, [_DATASETS_INFORMATION[dataset.name].height, _DATASETS_INFORMATION[dataset.name].width,3])
  else:
      image, height, width, seg = data_provider.get([common.IMAGE, common.HEIGHT, common.WIDTH, 'seg'])
  ...
  if dataset.name == 'apolloscape':
    ...
    if dataset_split != common.TEST_SET:
        ...
        shape_id_map_gt_masked = tf.where(tf.tile(mask, [1, 1, tf.shape(shape_id_map_gt)[2]]), shape_id_map_gt-1, tf.zeros_like(shape_id_map_gt)) # Gt is written as one plus
    ...
    if dataset_split != common.TEST_SET:
        ...
        return image, image_name, height, width, seg, mask
...
def get(dataset,
  ...
  if dataset_split != common.TEST_SET:
    ...
    image, image_name, height, width, seg, mask = _get_data(dataset, data_provider, dataset_split, codes_cons)
  ...
  if dataset_split != common.TEST_SET:
    ...
    sample = {
      ...
      'pose_map': label,
      'shape_map': shape_map,
      'label_pose_shape_map': pose_shape_map,
      'shape_id_map': shape_id_map,
      'shape_id_map_gt': shape_id_map_gt,
      'seg': seg,
      'mask': mask}
  else:
    sample = {
