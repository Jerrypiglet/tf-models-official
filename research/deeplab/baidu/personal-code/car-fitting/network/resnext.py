...
def resnext(data, units, num_stages, filter_list, num_classes, num_group,
            image_shape, bottle_neck=True, bn_mom=0.9, workspace=256,
            dtype='float32', domain=None, memonger=False):
    ...
    if height <= 32:            # such as cifar10
        ...
        body = mx.sym.Convolution(data=data,
                num_filter=filter_list[0], kernel=(7, 7), stride=(2,2), pad=(3, 3),
                no_bias=True, name="conv0", workspace=workspace)
    ...
    if isinstance(num_classes, list):
        output = []
        if domain is None:
            domain = ['%s_%d' % (i, n) for i, n in enumerate(num_classes)]

        for num_class, domain_name in zip(num_classes, domain):
            fc = mx.sym.FullyConnected(data=flat, num_hidden=num_class,
                    name='fc_%s' % domain_name)
            output.append(fc)

        return output

    else:
        fc1 = mx.sym.FullyConnected(data=flat, num_hidden=num_classes, name='fc_pose')
        return fc1
